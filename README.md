...
**`index.html`** — an interactive, browser-based visualisation of the model. Built for non-technical stakeholders (head teachers, Designated Safeguarding Leads, Behavioural Leads) to directly explore how three governance architectures behave under adjustable operational conditions, in real time, without needing to read code or statistics first. This is a single deterministic run per session, not a statistical study. Live at [grid-simulation.netlify.app](https://grid-simulation.netlify.app).
...
Statistical results reported in the paper's Results section are generated exclusively by `grid_abm.py`. The interactive dashboard (`index.html`) implements the same step logic for demonstration and stakeholder engagement, and is not itself the source of any reported statistic. All simulation inputs and outputs are synthetic; no real student, staff, or school data is used or stored in any artefact in this repository.
